...
class DSBD_NET(nn.Module):
    def __init__(self, noise_sig, ker_size=5):
        super(DSBD_NET, self).__init__()
        features = ker_size ** 2
        # self.wave_level = 3
...
if __name__ == '__main__':
    ...
    max_iterations = 1
    # the kernel size, then the dimension of kernel is kernel_size x kernel_size
    # the number of kernels =  kernel_size x kernel_size
    kernel_size = 5
    ...
    model = DSBD_NET(noise_sig=sigma_, ker_size=kernel_size)
    # print('model', model)
    model.cuda()

    optimizer = torch.optim.Adam(model.parameters(), lr=lrate)
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=2000, gamma=9e-1)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[key_step], gamma=gamma_lr)
    # scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=1e-4, max_lr=1e-3,step_size_up=1,mode="triangular")
